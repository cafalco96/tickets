...
## Fuente local opcional `Ticket Mono`

La apariencia simula una fuente monoespaciada tipo ESC/POS; no se afirma que ninguna fuente web sea idéntica a la de la impresora térmica de referencia. Por defecto se usa `Courier New`. Para usar una fuente propia:

1. Coloque los archivos en `public/fonts/` como `TicketMono.woff2` y/o `TicketMono.ttf`.
2. Seleccione **Ticket Mono** en **Configuración → Apariencia térmica y tipografía**.

Si el archivo no existe, el navegador usa el fallback automáticamente.
